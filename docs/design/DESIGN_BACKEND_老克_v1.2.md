...
5. token 过期前端处理口径统一为“弹登录弹窗，登录后停留当前页”。
...
- 不再保留 `execution_lock`
- `style` 建议直接存 PRD 枚举值：`2D次元风 / 日漫风 / 国漫风 / 古风 / 现代写实 / 自定义`，避免前后端再做一层码值映射
...
- Tika 负责解析 txt/word/md 为纯文本，纯文本结果另存 TOS。
